...
| [`proposed/design/`](proposed/design/) | Design for proposed features — `pointer-input.md` (F5), `device-growth.md` (F5), `hyperv-screen.md` (F5), `recorder.md` (F1, U6), `platform-dialect.md` (F64) |
| `pledged/design/` | Design for pledged features. A delivered feature leaves no feature for its design to sit with, so its design travels out with the delivery — to `docs/spec/` where the surface is world-facing, as landmarks' did, and to `design/` where it is internal |
