...
| [`pledged/design/`](pledged/design/) | Design for pledged features — `pointer-input.md` (F66); a delivered feature leaves no feature for its design to sit with, so its design travels out with the delivery — to `docs/spec/` where the surface is world-facing, as landmarks' did, and to `design/` where it is internal |
