...
| [`proposed/design/`](proposed/design/) | Design for proposed features — `device-growth.md` (F5), `hyperv-screen.md` (F5), `recorder.md` (F1, U6), `platform-dialect.md` (F64) |
| [`pledged/design/`](pledged/design/) | Design for pledged features — currently empty; a delivered feature leaves no feature for its design to sit with, so its design travels out with the delivery — to `docs/spec/` where the surface is world-facing, as landmarks' and pointer input's did, and to `design/` where it is internal |
| [`design/`](design/) | *(root)* Open design problems and internal doctrine belonging to no single feature — `backend-adapter.md` (the adapter seam's doctrine, delivered as F2 and internal by decision, so it does not move to `docs/spec/`), `screen-transcripts.md` (the capture format and its reconstruction, delivered as F42 and F43 and internal by D98, arriving here by exactly that route), `guest-communication.md` (the control-plane doctrine; the seam is internal, not world-facing), `authored-binary-assets.md` (where binary data an author supplies to a script lives — landmarks' settled shape, stated once because U25's fonts are its second instance), `audits.md` (suggestions for checking the project's own claims — ideas only, nothing enforced), `dosbox-x.md` (the DOSBox-X investigation record — why it was first ruled out, the personal-fork control channel that reopened it, and D120's adoption of it as the fifth backend), and the script-language residual problems in `script-examples/` |
