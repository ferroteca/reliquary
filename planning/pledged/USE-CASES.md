...
**U27 — Automate an installer that paints in its own font** — pledged
2026-08-19 (owner), cut out of U25 on review in the round that
pledged it (D109), and delivered by **F61** alone — the asset and the
statement serve both doors, and only the dump is U25's own. The
finding that separates them: **asking needs a moment when asking is
safe.** A dump is assembled by typing at a prompt, and an installer
paints its screens before any prompt exists in that boot — and those
moments are exactly the ones an install-testing script watches (U10),
so the door that serves a running guest cannot serve the screens that
drafted U25 in the first place. Here the author holds the face
already: off the installer's own media, from the vendor, or dumped
once from a guest of the same build. Where it came from is the
author's business and no part of this case; what Reliquary owes is
the reading. Text as pledged:
