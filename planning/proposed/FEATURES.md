...
## F35 — The recurring register, and the prose-parser migration

> Delivers the **P24 amendment**
> ([ARCHITECTURE.md](ARCHITECTURE.md), beside this file) and
> depends on it: pledging this feature without pledging the
> amendment would be work no decision demands. No use case asks
> for it. **Above the sprint bound as it stands** — the cut at
> pledge is per surface, each piece taking a fresh number and
> retiring this one (D42).

Deliverables:

1. **The register.** `planning/RECURRING.md`, its preamble
   stating the semantics the amendment argues: marks are state
   and never diary, a staleness bound is a ceiling and never an
...
2. **The initial obligations.** One prose-norm audit per spec
   document — grouping decided at pledge; ten norms sit in
   [docs/spec/](../../docs/spec/) today — and a
   dependency-freshness check (P21's rhythm: are the pins stale,
   does each dependency still pull its weight). Each with its
   own staleness bound.
3. **The migration**, module by module, under the amendment's
   no-second-registers rule — the corpus or a shipped schema
   where the machine artifact is or becomes the norm for what it
   captures, deletion with the norm's audit obligation
   inheriting the check otherwise:
   - `test_script_spec_conformance.py` — whole module;
     script-spec.md's V-rules already carry per-rule corpus
     fixtures, the natural machine home for rule coverage.
   - `test_api_spec_conformance.py` — the api.md table-row
     reads; the twin-parity half (`cli._COMMANDS` against
     `__all__`) reads no prose and stays.
   - `test_cli.py` `ClaimedCommandTests` — the docs/spec command
     sweep; the rest of the module stays.
   - `test_script_corpus.py` — the script-spec.md prefix-list
     read.
   - `test_conformance_corpus.py` — the instance-model.md
     phase-sentence read; the phase enum's machine home is
     `machine-state.schema.json` already.
   - `test_facts.py` — the script-properties.md catalog read.
   - `test_assets.py` — the asset-resolution.md extension-
     registry read.
   - `test_document.py` — the media-spec.md materialize-table
     read.
   - `test_library.py` — the codex.md inventory read.
4. **Root P24's text**, amended when the amendment itself
   delivers — the lifecycle's own move (D34) rather than extra
   work; named here so the piece that lands last knows it closes
   the arc.
