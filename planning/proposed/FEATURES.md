...
## F31 — Remanence owns at-rest disk access

> **Entered 2026-07-30** by the owner after evaluating the published
> `remanence==0.0.1a1` package against Reliquary's delivered at-rest
> access. Serves **U14**, **U20**; **P10**, **P11**, **P12**,
> **P16**, **P17**, **P21**. Design in
> [design/remanence-at-rest.md](design/remanence-at-rest.md).
>
> This is proposed rather than a task because the change replaces
> two internal layers, adds a runtime dependency, and cannot land
> until another project publishes capabilities it does not yet
> carry. It is not pledged, and nothing is worked from this entry.

Reliquary should consume Remanence as the one deep module for
opening raw and qcow2 drive images at rest, discovering their
partition and volume geometry, and reading or changing files in
their FAT volumes. The Reliquary-owned NBD client, `qemu-nbd`
server lifecycle, qcow2 snapshot orchestration, staged-raw access,
MBR reader, and FAT reader/writer then leave together. A hybrid
that keeps either implementation as a fallback is declined: it
would make the dependency additive rather than substitutive and
leave two authorities for the same disk facts, contrary to P21
and to D83's one-report ruling.

**THE DEPENDENCY EARNS THE WHOLE LAYER OR NONE OF IT.** Remanence's
published `Disk` interface is already shaped around the work:
format recognition, a deny-write claim, geometry, directory
entries, file reads and writes, and commit or rollback. Using only
its qcow2 byte path would delete the NBD transport while preserving
Reliquary's second implementation of every fact above it. The
deeper cut gives one module custody of format allocation, backing
chains, partition enumeration, filesystem mutation, locking, and
the commit point. Reliquary keeps the policy Remanence cannot own:
the DOS-only FAT12/FAT16/FAT16B recognition claim, guest-address
mapping, rule ids, and translation into the recorded drive report.

**THE CURRENT RELEASE IS EVIDENCE, NOT A CANDIDATE.**
`remanence==0.0.1a1` refuses qcow2 backing files, treats a blank
newly materialized disk as an unreadable filesystem, silently
omits unreadable partitions from `geometry()`, and supplies no
stable volume identity shared by `geometry()` and `entries()`.
Its buffered commit also does not establish D77's crash-recovery
guarantee. Pinning that release would make a differencing machine
fail at the first start-time geometry read and would regress the
blank-disk and interruption cases the current implementation
settled. The dependency is admitted only after a later Remanence
release meets the acceptance contract in the design, and that
exact release is pinned.

**NO APPLICATION SURFACE CHANGES.** This replaces implementation
below the backend adapter seam. S1 and S2 keep the same file and
drive operations, addresses, reports, and phase rules; S7 keeps
the same documents, exit outcomes, and Reliquary rule ids; S8
gains no stable path or user-owned artifact. The machine blueprint,
script language, properties, and codex are untouched. If landing
the dependency cannot preserve one of those statements, this
proposal must return for surface review rather than spending the
difference as integration detail.

**D73 AND D77 ARE BOTH HONORED.** D73 refused writing and
maintaining a qcow2 reader *inside Reliquary*; a declared
dependency is the route D82 requires for third-party source and is
the ownership move D73 wanted. D77's transport choice is
superseded, not its guarantees: an image is still opened where it
lies, a backing relationship survives a write, work is proportional
to touched data, contention fails by name, and an interrupted write
is reconciled on the next access. D83's geometry and D78's drive
letters remain observations read from the disk, never blueprint
declarations.

Delivery removes the old implementation in the same change that
adopts the dependency. The focused tests move to Remanence-facing
contract tests and real QEMU-authored qcow2 fixtures; Reliquary's
existing file, geometry, letter-map, lifecycle, and failure tests
remain the regression oracle. No release is accepted on its
interface description alone: P11 requires the claimed paths to run
on the delivered Windows host.
