...
**U25 — Automate a guest that paints in its own font** — drafted
(add, 2026-08-13, while fixing the fonts a FreeDOS install is read
through; the case text **reshaped the same day on review**, having
been written as a design argument rather than as something a user
could follow — the same fault U24 was pledged with and corrected
for). The gap: nothing in the list says a script can read a screen
whose glyphs came from the guest rather than the host, and the
recognizer's whole input is what the *host's* hypervisor binaries
carry. On pledge the display plane's own documentation cites U25,
and the storage half is already settled — see
[design/authored-binary-assets.md](../design/authored-binary-assets.md),
which this use case is the second instance of.
...
than any one script's business.

**But asking needs a moment when asking is safe**, and that bounds
the native door: a dump is assembled by typing at a prompt, and an
installer paints its screens before any prompt exists in that boot.
The door therefore serves the guest a script can reach, which is most
of a machine's working life, and cannot serve the screens that
drafted this — which is why the case names a second door, a font the
author supplies.
...
then the guest takes control and paints with its own. So a supplied
font is stated **positionally** — from here forward, try this first,
the rest behind it — the shape U24's `boot` settled for drive order:
name what comes first and let the remainder follow. Saying it also
**narrows**, which is the part that is easy to miss: every additional
bank is one more chance for a near-match to beat the true glyph, so
naming the font in force corrects priority rather than adding to a
pile.
...
**Open at this draft**, both deliberately, and both are why the case
above names no command: what a font's declaration states, the shape
around it being settled; and how the bytes leave the guest. The
...
>   prepared codepage, a localized installer — and the screen is
>   being read against the fonts the *host's* hypervisor installed,
>   which do not include it; the run ends at a timeout saying how
>   much of the screen could not be read. What the author needs is
>   the guest's own font among the ones the screen is read through:
>   **taken from the guest** where there is a moment to ask it for
>   one, **kept beside the script** as an asset of its own where
>   there is not, and **named from the point in the run where the
>   guest takes the screen over** — the firmware painted the earlier
>   screens, and painted them in a different face. From there the
>   wait matches the text that was on the screen all along, and the
>   guest automates on any host that will run it (U7).
