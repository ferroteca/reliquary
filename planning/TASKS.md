...
#### T28 — `wait_ready` recognizes only the standard prompt, and it is the documented readiness idiom

`AgentlessGuestExec.wait_ready()` is the embedding API's readiness
wait — exported from the package root, declared in
[docs/api-reference.md](../docs/api-reference.md), and shown as
*the* idiom before `execute` in [README.md](../README.md) and
[docs/dos-automation.md](../docs/dos-automation.md). It recognizes
the standard DOS prompt alone, `X:\path>`. **D112** gave `execute`
a second source of evidence — exactly the prompt the guest was
sitting at when the command was sent — and left `wait_ready` with
the first only, because it has no earlier screen to read a
customized prompt off. So a guest whose `AUTOEXEC.BAT` sets
`PROMPT [$P]$G` now runs every command through `execute` and still
never reports ready: the documented idiom fails at its first line,
on the public surface, for the guest D112 just made usable.

**The question is where readiness learns a prompt from, with no
prior screen to observe.** The candidates D112 weighed for
`execute` return here with the balance changed: a declared prompt
pattern was declined *there* on no demand and because the screen
already said what it needed; *here* the screen cannot say it, and
the demand is the idiom the project's own guides teach. The other
candidate is to settle what readiness means without a pattern —
a screen that has stopped changing with *some* bottom row, the
stability machinery already in the adapter — which D112's
false-positive argument (any row ending in `>`) bears on directly.
Either way the answer is a surface decision (S2, the embedding
API; S4 if a blueprint field carries it) under
[SURFACES.md](SURFACES.md), recorded as a D-number, and the guides
follow it.

Filed 2026-08-22 from the D112 round, which first mis-stated this
method as uncalled: it is uncalled inside `src/`, and called by
every program the documentation tells how to write.
