...
> world-facing interfaces. The interfaces themselves are
> enumerated in root [ARCHITECTURE.md](../ARCHITECTURE.md) "The
> interfaces" — that enumeration is this rule's scope, answered
> by lookup — and each surface's normative specification lives in
> [docs/spec/](../docs/spec/). This document says how an
> interface-changing decision is weighed: against the use cases
...
any surface the enumeration names is automatically not
...
root [ARCHITECTURE.md](../ARCHITECTURE.md) "The interfaces"
enumerates the surfaces, so it is a checklist, not a
judgement. **The norm is part of the interface** (P23): an edit
to a normative artifact — a `docs/spec/` specification, the
...
own merits — the amendment is the argument, and the interface
...
The specification each interface answers to is indexed in
