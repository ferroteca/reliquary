...
The module-level launch and one-shot workflow functions gain a trailing,
optional `machine_config=None` keyword accepting a `MachineConfig`, mapping,
or path-like object. Existing individual parameters remain valid. Supplying a
setting both through `machine_config` and an existing explicit function
parameter is rejected when the values differ; relict does not silently guess
which representation the caller intended to win.

The QEMU executable remains `MachineConfig.qemu` or the existing `qemu`
function parameter. It identifies the program used to realize the machine,
not the machine's emulated hardware type.
...
loading, validation, path rules, compatibility, and argument construction
...
6. Existing direct-function parameters override the selected Python
   configuration when explicitly supplied.
7. Lifecycle-owned identity and QMP arguments are always generated by relict
...
With no `machine.json` and no explicit configuration, behavior and arguments
remain unchanged. Files under `drives/`, existing public fields, and
direct-use functions remain supported.
