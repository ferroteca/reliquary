...
The glyph bank is Reliquary's own CP437-layout 8×16 font
(``fonts/cp437_8x16.bin``), generated by ``tools/gen_cp437_font.py``
— original work, not a third-party ROM dump. Fixtures are rendered
with the same bank, so the default suite reconstructs known screens
with no hypervisor.
"""
